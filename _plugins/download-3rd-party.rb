Jekyll::Hooks.register :site, :after_init do |site|
  ...
  skip_download = ['1', 'true', 'yes'].include?(ENV['AL_FOLIO_SKIP_3RD_PARTY_DOWNLOAD'].to_s.downcase)
  download_on_serve = ['1', 'true', 'yes'].include?(ENV['AL_FOLIO_DOWNLOAD_3RD_PARTY_ON_SERVE'].to_s.downcase)
  is_serving = site.config['serving'] == true
  skip_download = skip_download || (is_serving && !download_on_serve)

  # download 3rd party libraries if required
  if site.config['third_party_libraries']['download'] && !skip_download
    site.config['third_party_libraries'].each do |key, value|
      if key != 'download'
        value['url'].each do |type, url|
          # check if url is a dictionary
          if url.is_a?(Hash)
            ...
            if type == 'fonts'
              ...
              local_dir = site.config.dig('third_party_libraries', key, 'local', type)

              is_css = file_name.end_with?('css') || url.include?('.css') || url.include?('fonts.googleapis.com') || file_name.start_with?('css')
              if is_css
                ...
                download_fonts(url, File.join(site.source, 'assets', 'libs', key, local_dir), font_file_types)
              else
                puts "Skipping font download for #{key} because third_party_libraries.#{key}.local.#{type} is not configured"
              end
              ...
              if local_dir
                ...
                download_images(url, File.join(site.source, 'assets', 'libs', key, local_dir), image_file_types)
              else
                puts "Skipping image download for #{key} because third_party_libraries.#{key}.local.#{type} is not configured"
              end
  ...
  elsif is_serving && !download_on_serve
    puts "Skipping 3rd party downloads during `jekyll serve`"
  elsif skip_download
    puts "Skipping 3rd party downloads because AL_FOLIO_SKIP_3RD_PARTY_DOWNLOAD is set"
  end
end
